...
V1 refuses to overwrite a different existing role and leaves the code unused.
...
This adds to `address_sender.balance` for the target address entered on the redemption page. Credits belong to an email address. Redemption does not re-enable an address disabled by an administrator.
