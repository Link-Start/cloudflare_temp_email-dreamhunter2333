...
if [ -n "${WORKER_URL_SITE_PASSWORD:-}" ]; then
  ...
  curl --connect-timeout 5 --max-time 10 -sf -H "x-custom-auth: e2e-site-pass" -X POST "$WORKER_URL_SITE_PASSWORD/admin/db_initialize" > /dev/null
  curl --connect-timeout 5 --max-time 10 -sf -H "x-custom-auth: e2e-site-pass" -X POST "$WORKER_URL_SITE_PASSWORD/admin/db_migration" > /dev/null
